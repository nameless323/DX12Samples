...
GaussBlur::GaussBlur(HINSTANCE hInstance) : Application(hInstance)
{
}
...
bool GaussBlur::Init()
{
    ...
    _blurFilter = std::make_unique<GaussBlurFilter>(_device.Get(), _clientWidth, _clientHeight, DXGI_FORMAT_R8G8B8A8_UNORM);
...
LRESULT GaussBlur::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
...
void GaussBlur::OnResize()
{
    ...
    if(_blurFilter != nullptr)
        _blurFilter->OnResize(_clientWidth, _clientHeight);
}

void GaussBlur::Update(const GameTimer& timer)
{
...
void GaussBlur::Draw(const GameTimer& timer)
{
    ...
    _blurFilter->Execute(_commandList.Get(), _postProcessRootSignature.Get(), _PSOs["horizBlur"].Get(), _PSOs["vertBlur"].Get(), CurrentBackBuffer(), 4);

    _commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(), D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_COPY_DEST));

    _commandList->CopyResource(CurrentBackBuffer(), _blurFilter->Output());

    _commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PRESENT));
...
void GaussBlur::OnMouseDown(WPARAM btnState, int x, int y)
{
    _lastMousePos.x = x;
    _lastMousePos.y = y;
...
void GaussBlur::OnMouseUp(WPARAM btnState, int x, int y)
{
    ReleaseCapture();
}

void GaussBlur::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void GaussBlur::OnKeyboardInput(const GameTimer& timer)
{
...
void GaussBlur::UpdateCamera(const GameTimer& timer)
{
...
void GaussBlur::AnimateMaterials(const GameTimer& timer)
{
...
void GaussBlur::UpdateObjectCBs(const GameTimer& timer)
{
...
void GaussBlur::UpdateMaterialCBs(const GameTimer& timer)
{
...
void GaussBlur::UpdateMainPassCB(const GameTimer& timer)
{
...
void GaussBlur::UpdateWaves(const GameTimer& timer)
{
...
void GaussBlur::BuildRootSignature()
{
...
void GaussBlur::BuildPostProcessRootSignature()
{
...
void GaussBlur::BuildDescriptorHeaps()
{
    ...
    _blurFilter->BuildDescriptors(CD3DX12_CPU_DESCRIPTOR_HANDLE(_srvHeap->GetCPUDescriptorHandleForHeapStart(), 3, _cbvSrvUavDescriptorSize),
        CD3DX12_GPU_DESCRIPTOR_HANDLE(_srvHeap->GetGPUDescriptorHandleForHeapStart(), 3, _cbvSrvUavDescriptorSize),
        _cbvSrvUavDescriptorSize);
}

void GaussBlur::BuildShaderAndInputLayout()
{
    ...
    _shaders["horizBlurCS"] = D3DUtil::CompileShader(L"Shaders\\GaussBlur.hlsl", nullptr, "HorizBlurCS", "cs_5_0");
    _shaders["vertBlurCS"] = D3DUtil::CompileShader(L"Shaders\\GaussBlur.hlsl", nullptr, "VertBlurCS", "cs_5_0");
...
void GaussBlur::BuildLandGeometry()
{
...
void GaussBlur::BuildWavesGeometry()
{
...
void GaussBlur::BuildBoxGeometry()
{
    GeometryGenerator geoGen;
...
void GaussBlur::BuildPSOs()
{
    ...
    D3D12_COMPUTE_PIPELINE_STATE_DESC horizBlurPSO = {};
    horizBlurPSO.pRootSignature = _postProcessRootSignature.Get();
    horizBlurPSO.CS =
    {
        _shaders["horizBlurCS"]->GetBufferPointer(),
        _shaders["horizBlurCS"]->GetBufferSize()
    };
    horizBlurPSO.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
    ThrowIfFailed(_device->CreateComputePipelineState(&horizBlurPSO, IID_PPV_ARGS(&_PSOs["horizBlur"])));

    D3D12_COMPUTE_PIPELINE_STATE_DESC vertBlurPSO = {};
    vertBlurPSO.pRootSignature = _postProcessRootSignature.Get();
    vertBlurPSO.CS =
    {
        _shaders["vertBlurCS"]->GetBufferPointer(),
        _shaders["vertBlurCS"]->GetBufferSize()
    };
    vertBlurPSO.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
    ThrowIfFailed(_device->CreateComputePipelineState(&vertBlurPSO, IID_PPV_ARGS(&_PSOs["vertBlur"])));
}

void GaussBlur::BuildFrameResources()
{
...
void GaussBlur::BuildRenderItems()
{
...
void GaussBlur::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& renderItems)
{
...
float GaussBlur::GetHillsHeight(float x, float z) const
{
    return 0.3f*(z*sinf(0.1f*x) + x*cosf(0.1f*z));
}

XMFLOAT3 GaussBlur::GetHillsNormal(float x, float z) const
{
    XMFLOAT3 n(
