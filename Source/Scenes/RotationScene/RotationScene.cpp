...
using Vertex = RotationSceneFrameResource::Vertex;
using FrameResource = RotationSceneFrameResource;
using RenderLayer = RenderItem::RenderLayer;

RotationScene::RotationScene(HINSTANCE hInstance) : Application(hInstance)
{
}

bool RotationScene::Init()
{
    if (!Application::Init())
        return false;
...
LRESULT RotationScene::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    return Application::MsgProc(hwnd, msg, wParam, lParam);
}

int RotationScene::Run()
{
    return Application::Run();
}

void RotationScene::OnResize()
{
...
void RotationScene::Update(const GameTimer& timer)
{
    OnKeyboardInput(timer);

    _animTimePos += timer.DeltaTime();
    if (_animTimePos >= _skullAnimation.GetEndTime())
        _animTimePos = 0.0f;
    _skullAnimation.Interpolate(_animTimePos, _skullModel);
    _skullRenderItem->Model = _skullModel;    
...
void RotationScene::Draw(const GameTimer& timer)
{
...
void RotationScene::OnMouseDown(WPARAM btnState, int x, int y)
{
    _lastMousePos.x = x;
    _lastMousePos.y = y;
...
void RotationScene::OnMouseUp(WPARAM btnState, int x, int y)
{
    ReleaseCapture();
}

void RotationScene::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void RotationScene::OnKeyboardInput(const GameTimer& timer)
{
...
void RotationScene::AnimateMaterials(const GameTimer& timer)
{
}

void RotationScene::UpdateObjectCBs(const GameTimer& timer)
{
...
void RotationScene::UpdateMaterialBuffer(const GameTimer& timer)
{
...
void RotationScene::UpdateMainPassCB(const GameTimer& timer)
{
...
void RotationScene::DefineScullAnimation()
{
...
void RotationScene::LoadTextures()
{
...
void RotationScene::BuildRootSignature()
{
...
void RotationScene::BuildDescriptorHeaps()
{
...
void RotationScene::BuildShaderAndInputLayout()
{
    _shaders["standardVS"] = D3DUtil::CompileShader(L"Shaders\\RotationDefault.hlsl", nullptr, "vert", "vs_5_1");
    _shaders["opaquePS"] = D3DUtil::CompileShader(L"Shaders\\RotationDefault.hlsl", nullptr, "frag", "ps_5_1");
...
void RotationScene::BuildShapeGeometry()
{
...
void RotationScene::BuildSkullGeometry()
{
...
void RotationScene::BuildFrameResources()
{
...
void RotationScene::BuildMaterials()
{
    ...
    _materials["crate0"] = std::move(crate0);
    _materials["skullMat"] = std::move(skullMat);
}

void RotationScene::BuildRenderItems()
{
    ...
    XMStoreFloat4x4(&boxRitem->Model, XMMatrixScaling(3.0f, 1.0f, 3.0f)*XMMatrixTranslation(0.0f, 0.5f, 0.0f));
...
void RotationScene::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& renderItems)
{
