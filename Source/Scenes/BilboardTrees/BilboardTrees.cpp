...
BilboardTrees::BilboardTrees(HINSTANCE hInstance) : Application(hInstance)
{
}

BilboardTrees::~BilboardTrees()
{
    if (_device != nullptr)
        FlushCommandQueue();
}

bool BilboardTrees::Init()
{
    if (!Application::Init())
        return false;
...
LRESULT BilboardTrees::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    return Application::MsgProc(hwnd, msg, wParam, lParam);
}

int BilboardTrees::Run()
{
    return Application::Run();
}

void BilboardTrees::OnResize()
{
...
void BilboardTrees::Update(const GameTimer& timer)
{
...
void BilboardTrees::Draw(const GameTimer& timer)
{
    ...
    _commandList->SetPipelineState(_PSOs["treeSprites"].Get());
    DrawRenderItems(_commandList.Get(), _renderItemLayer[(int)RenderItem::RenderLayer::AlphaTestedTreeSprites]);
...
void BilboardTrees::OnMouseDown(WPARAM btnState, int x, int y)
{
    _lastMousePos.x = x;
    _lastMousePos.y = y;
...
void BilboardTrees::OnMouseUp(WPARAM btnState, int x, int y)
{
    ReleaseCapture();
}

void BilboardTrees::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void BilboardTrees::OnKeyboardInput(const GameTimer& timer)
{
...
void BilboardTrees::UpdateCamera(const GameTimer& timer)
{
...
void BilboardTrees::AnimateMaterials(const GameTimer& timer)
{
...
void BilboardTrees::UpdateObjectCBs(const GameTimer& timer)
{
...
void BilboardTrees::UpdateMaterialCBs(const GameTimer& timer)
{
...
void BilboardTrees::UpdateMainPassCB(const GameTimer& timer)
{
...
void BilboardTrees::UpdateWaves(const GameTimer& timer)
{
...
void BilboardTrees::LoadTextures()
{
    ...
    ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(_device.Get(), _commandList.Get(), treeArrayTex->Filename.c_str(), treeArrayTex->Resource, treeArrayTex->UploadHeap));

    _textures[grassTex->Name] = move(grassTex);
    _textures[waterTex->Name] = move(waterTex);
    _textures[fenceTex->Name] = move(fenceTex);
    _textures[treeArrayTex->Name] = move(treeArrayTex);
}

void BilboardTrees::BuildRootSignature()
{
...
void BilboardTrees::BuildDescriptorHeaps()
{
    ...
    auto treeArrayTex = _textures["treeArrayTex"]->Resource;
    ...
    _device->CreateShaderResourceView(treeArrayTex.Get(), &srvDesc, hDescriptor);
}

void BilboardTrees::BuildShaderAndInputLayout()
{
    ...
    _shaders["treeSpriteVS"] = D3DUtil::CompileShader(L"Shaders\\TreeSprite.hlsl", nullptr, "vert", "vs_5_1");
    _shaders["treeSpriteGS"] = D3DUtil::CompileShader(L"Shaders\\TreeSprite.hlsl", nullptr, "geom", "gs_5_1");
    _shaders["treeSpritePS"] = D3DUtil::CompileShader(L"Shaders\\TreeSprite.hlsl", alphaTestDefines, "frag", "ps_5_1");
...
void BilboardTrees::BuildLandGeometry()
{
...
void BilboardTrees::BuildWavesGeometry()
{
...
void BilboardTrees::BuildBoxGeometry()
{
    GeometryGenerator geoGen;
...
void BilboardTrees::BuildTreeSpritesGeometry()
{
    ...
    _geometries["treeSpritesGeo"] = move(geo);
}

void BilboardTrees::BuildPSOs()
{
    ...
    {
        reinterpret_cast<BYTE*>(_shaders["treeSpriteVS"]->GetBufferPointer()),
        _shaders["treeSpriteVS"]->GetBufferSize()
    };
    treeSpritePsoDesc.GS =
    {
        reinterpret_cast<BYTE*>(_shaders["treeSpriteGS"]->GetBufferPointer()),
        _shaders["treeSpriteGS"]->GetBufferSize()
    };
    treeSpritePsoDesc.PS =
    {
        reinterpret_cast<BYTE*>(_shaders["treeSpritePS"]->GetBufferPointer()),
        _shaders["treeSpritePS"]->GetBufferSize()
    };
...
void BilboardTrees::BuildFrameResources()
{
...
void BilboardTrees::BuildMaterials()
{
    ...
    _materials["treeSprites"] = move(treeSprites);
}

void BilboardTrees::BuildRenderItems()
{
    ...
    treeSpritesRenderItem->Model = MathHelper::Identity4x4();
    treeSpritesRenderItem->ObjCBIndex = 3;
    treeSpritesRenderItem->Mat = _materials["treeSprites"].get();
    treeSpritesRenderItem->Geo = _geometries["treeSpritesGeo"].get();
    ...
    _renderItemLayer[(int)RenderItem::RenderLayer::AlphaTestedTreeSprites].push_back(treeSpritesRenderItem.get());
    ...
    _allRenderItems.push_back(move(treeSpritesRenderItem));
}
...
float BilboardTrees::GetHillsHeight(float x, float z) const
{
    return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 BilboardTrees::GetHillsNormal(float x, float z) const
{
    XMFLOAT3 n(
