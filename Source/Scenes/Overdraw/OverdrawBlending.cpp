...
OverdrawBlending::OverdrawBlending(HINSTANCE hInstance) : Application(hInstance)
{
}

OverdrawBlending::~OverdrawBlending()
{
    if (_device != nullptr)
        FlushCommandQueue();
}

bool OverdrawBlending::Init()
{
    if (!Application::Init())
        return false;
...
LRESULT OverdrawBlending::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    return Application::MsgProc(hwnd, msg, wParam, lParam);
}

int OverdrawBlending::Run()
{
    return Application::Run();
}

void OverdrawBlending::OnResize()
{
...
void OverdrawBlending::Update(const GameTimer& timer)
{
...
void OverdrawBlending::Draw(const GameTimer& timer)
{
    ...
    {
        ThrowIfFailed(_commandList->Reset(cmdListAlloc.Get(), _PSOs["overdraw"].Get()));
    }
    ...
    if (!_showOverdraw)
        ...
        _commandList->SetPipelineState(_PSOs["overdrawAlphaTested"].Get());
...
void OverdrawBlending::OnMouseDown(WPARAM btnState, int x, int y)
{
    _lastMousePos.x = x;
    _lastMousePos.y = y;
...
void OverdrawBlending::OnMouseUp(WPARAM btnState, int x, int y)
{
    ReleaseCapture();
}

void OverdrawBlending::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void OverdrawBlending::OnKeyboardInput(const GameTimer& timer)
{
...
void OverdrawBlending::UpdateCamera(const GameTimer& timer)
{
...
void OverdrawBlending::AnimateMaterials(const GameTimer& timer)
{
...
void OverdrawBlending::UpdateObjectCBs(const GameTimer& timer)
{
...
void OverdrawBlending::UpdateMaterialCBs(const GameTimer& timer)
{
...
void OverdrawBlending::UpdateMainPassCB(const GameTimer& timer)
{
...
void OverdrawBlending::UpdateWaves(const GameTimer& timer)
{
...
void OverdrawBlending::LoadTextures()
{
...
void OverdrawBlending::BuildRootSignature()
{
...
void OverdrawBlending::BuildDescriptorHeaps()
{
...
void OverdrawBlending::BuildShaderAndInputLayout()
{
    ...
    _shaders["overdraw"] = D3DUtil::CompileShader(L"Shaders\\BlendingOverdraw.hlsl", nullptr, "frag", "ps_5_1");
    _shaders["overdrawAlphaTested"] = D3DUtil::CompileShader(L"Shaders\\BlendingOverdraw.hlsl", alphaTestDefines, "frag", "ps_5_1");
...
void OverdrawBlending::BuildLandGeometry()
{
...
void OverdrawBlending::BuildWavesGeometry()
{
...
void OverdrawBlending::BuildBoxGeometry()
{
    GeometryGenerator geoGen;
...
void OverdrawBlending::BuildPSOs()
{
    ...
    D3D12_RENDER_TARGET_BLEND_DESC overdrawBlendDesc = transparentBlendDesc;
    overdrawBlendDesc.SrcBlend = D3D12_BLEND_ONE;
    overdrawBlendDesc.DestBlend = D3D12_BLEND_ONE;

    D3D12_GRAPHICS_PIPELINE_STATE_DESC overdrawPsoDesc = transparentPsoDesc;
    overdrawPsoDesc.BlendState.RenderTarget[0] = overdrawBlendDesc;
    overdrawPsoDesc.PS =
    {
    ...
    D3D12_GRAPHICS_PIPELINE_STATE_DESC overdrawAlphaTestedPsoDesc = overdrawPsoDesc;
    overdrawAlphaTestedPsoDesc.PS =
    {
        reinterpret_cast<BYTE*>(_shaders["overdrawAlphaTested"]->GetBufferPointer()),
        _shaders["overdrawAlphaTested"]->GetBufferSize()
    };
    overdrawAlphaTestedPsoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;
    ThrowIfFailed(_device->CreateGraphicsPipelineState(&overdrawAlphaTestedPsoDesc, IID_PPV_ARGS(&_PSOs["overdrawAlphaTested"])));

}

void OverdrawBlending::BuildFrameResources()
{
...
void OverdrawBlending::BuildMaterials()
{
...
void OverdrawBlending::BuildRenderItems()
{
...
void OverdrawBlending::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& renderItems)
{
...
float OverdrawBlending::GetHillsHeight(float x, float z) const
{
    return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 OverdrawBlending::GetHillsNormal(float x, float z) const
{
    XMFLOAT3 n(
