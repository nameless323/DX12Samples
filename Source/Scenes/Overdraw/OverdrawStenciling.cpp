...
OverdrawStenciling::OverdrawStenciling(HINSTANCE hInstance) : Application(hInstance)
{
}

OverdrawStenciling::~OverdrawStenciling()
{
    if (_device != nullptr)
        FlushCommandQueue();
}

bool OverdrawStenciling::Init()
{
    if (!Application::Init())
        return false;
...
LRESULT OverdrawStenciling::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    return Application::MsgProc(hwnd, msg, wParam, lParam);
}

int OverdrawStenciling::Run()
{
    return Application::Run();
}

void OverdrawStenciling::OnResize()
{
...
void OverdrawStenciling::Update(const GameTimer& timer)
{
...
void OverdrawStenciling::Draw(const GameTimer& timer)
{
    ...
    if (_showOverdraw)
    {
        _commandList->SetPipelineState(_PSOs["overdraw"].Get());
        _commandList->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
        _commandList->IASetVertexBuffers(0, 1, &_geometries["boxGeo"]->VertexBufferView());
        _commandList->IASetIndexBuffer(&_geometries["boxGeo"]->IndexBufferView());
        _commandList->ClearRenderTargetView(CurrentBackBufferView(), (float*)&Colors::Green, 0, nullptr);
        for (UINT i = 0; i < 10; i++)
        {
            _commandList->OMSetStencilRef(i);
            _commandList->SetGraphicsRoot32BitConstant(4, i, 0);
...
void OverdrawStenciling::OnMouseDown(WPARAM btnState, int x, int y)
{
    _lastMousePos.x = x;
    _lastMousePos.y = y;
...
void OverdrawStenciling::OnMouseUp(WPARAM btnState, int x, int y)
{
    ReleaseCapture();
}

void OverdrawStenciling::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void OverdrawStenciling::OnKeyboardInput(const GameTimer& timer)
{
...
void OverdrawStenciling::UpdateCamera(const GameTimer& timer)
{
...
void OverdrawStenciling::AnimateMaterials(const GameTimer& timer)
{
...
void OverdrawStenciling::UpdateObjectCBs(const GameTimer& timer)
{
...
void OverdrawStenciling::UpdateMaterialCBs(const GameTimer& timer)
{
...
void OverdrawStenciling::UpdateMainPassCB(const GameTimer& timer)
{
...
void OverdrawStenciling::UpdateWaves(const GameTimer& timer)
{
...
void OverdrawStenciling::LoadTextures()
{
...
void OverdrawStenciling::BuildRootSignature()
{
...
void OverdrawStenciling::BuildDescriptorHeaps()
{
...
void OverdrawStenciling::BuildShaderAndInputLayout()
{
    ...
    _shaders["standardVS"] = D3DUtil::CompileShader(L"Shaders\\Blending.hlsl", nullptr, "vert", "vs_5_1");
    _shaders["opaquePS"] = D3DUtil::CompileShader(L"Shaders\\Blending.hlsl", defines, "frag", "ps_5_1");
    _shaders["alphaTestedPS"] = D3DUtil::CompileShader(L"Shaders\\Blending.hlsl", alphaTestDefines, "frag", "ps_5_1");

    _shaders["overdraw"] = D3DUtil::CompileShader(L"Shaders\\StencilingOverdraw.hlsl", nullptr, "frag", "ps_5_1");
    _shaders["overdrawVS"] = D3DUtil::CompileShader(L"Shaders\\StencilingOverdraw.hlsl", nullptr, "vert", "vs_5_1");
...
void OverdrawStenciling::BuildLandGeometry()
{
...
void OverdrawStenciling::BuildWavesGeometry()
{
...
void OverdrawStenciling::BuildBoxGeometry()
{
    GeometryGenerator geoGen;
...
void OverdrawStenciling::BuildPSOs()
{
    D3D12_DEPTH_STENCIL_DESC opaqueDepthStencilDesc;
    opaqueDepthStencilDesc.DepthEnable = true;
    opaqueDepthStencilDesc.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ALL;
    opaqueDepthStencilDesc.DepthFunc = D3D12_COMPARISON_FUNC_LESS;

    opaqueDepthStencilDesc.StencilEnable = true;
    opaqueDepthStencilDesc.StencilReadMask = 0xff;
    opaqueDepthStencilDesc.StencilWriteMask = 0xff;
    
    opaqueDepthStencilDesc.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;
    opaqueDepthStencilDesc.FrontFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
    opaqueDepthStencilDesc.FrontFace.StencilFailOp = D3D12_STENCIL_OP_INCR;
    opaqueDepthStencilDesc.FrontFace.StencilPassOp = D3D12_STENCIL_OP_INCR;

    opaqueDepthStencilDesc.BackFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;
    opaqueDepthStencilDesc.BackFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
    opaqueDepthStencilDesc.BackFace.StencilFailOp = D3D12_STENCIL_OP_INCR;
    opaqueDepthStencilDesc.BackFace.StencilPassOp = D3D12_STENCIL_OP_INCR;
    ...
    opaquePsoDesc.DepthStencilState = opaqueDepthStencilDesc;
    ...
    D3D12_DEPTH_STENCIL_DESC overdrawDepthStencilDesc = opaqueDepthStencilDesc;

    overdrawDepthStencilDesc.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;
    overdrawDepthStencilDesc.FrontFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
    overdrawDepthStencilDesc.FrontFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
    overdrawDepthStencilDesc.FrontFace.StencilPassOp = D3D12_STENCIL_OP_KEEP;

    overdrawDepthStencilDesc.BackFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;
    overdrawDepthStencilDesc.BackFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
    overdrawDepthStencilDesc.BackFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
    overdrawDepthStencilDesc.BackFace.StencilPassOp = D3D12_STENCIL_OP_KEEP;
    D3D12_GRAPHICS_PIPELINE_STATE_DESC overdrawPsoDesc = opaquePsoDesc;
    overdrawPsoDesc.DepthStencilState = overdrawDepthStencilDesc;
    overdrawPsoDesc.PS =
    {
        reinterpret_cast<BYTE*>(_shaders["overdraw"]->GetBufferPointer()),
        _shaders["overdraw"]->GetBufferSize()
    };
    overdrawPsoDesc.VS =
    {
        reinterpret_cast<BYTE*>(_shaders["overdrawVS"]->GetBufferPointer()),
        _shaders["overdrawVS"]->GetBufferSize()
    };
    ThrowIfFailed(_device->CreateGraphicsPipelineState(&overdrawPsoDesc, IID_PPV_ARGS(&_PSOs["overdraw"])));


}

void OverdrawStenciling::BuildFrameResources()
{
...
void OverdrawStenciling::BuildMaterials()
{
...
void OverdrawStenciling::BuildRenderItems()
{
...
void OverdrawStenciling::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& renderItems)
{
...
float OverdrawStenciling::GetHillsHeight(float x, float z) const
{
    return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 OverdrawStenciling::GetHillsNormal(float x, float z) const
{
    XMFLOAT3 n(
