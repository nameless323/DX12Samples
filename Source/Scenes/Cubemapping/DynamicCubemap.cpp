...
using PassConstants = CubemappingFrameResource::PassConstants;

DynamicCubemap::DynamicCubemap(HINSTANCE hInstance) : Application(hInstance)
{
}
...
bool DynamicCubemap::Init()
{
    ...
    _dynamicCubemap = std::make_unique<CubeRenderTarget>(_device.Get(), CubeMapSize, CubeMapSize, DXGI_FORMAT_R8G8B8A8_UNORM);
...
LRESULT DynamicCubemap::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    return Application::MsgProc(hwnd, msg, wParam, lParam);
}

int DynamicCubemap::Run()
{
...
void DynamicCubemap::Update(const GameTimer& timer)
{
    ...
    XMStoreFloat4x4(&_skullRenderItem->Model, skullScale * skullLocalRotate * skullOffset * skullGlobalRotate);
    _skullRenderItem->NumFramesDirty = FrameResource::NumFrameResources;
...
void DynamicCubemap::Draw(const GameTimer& timer)
{
    ...
    _commandList->SetGraphicsRootDescriptorTable(3, dynamicTexDescriptor);

    DrawRenderItems(_commandList.Get(), _renderItemLayer[(int)RenderLayer::OpaqueDynamicReflectors]);
    _commandList->SetGraphicsRootDescriptorTable(3, skyTexDescriptor);
...
void DynamicCubemap::CreateRtvAndDsvDescriptorHeaps()
{
    ...
    rtvHeapDesc.NumDescriptors = _swapChainBufferCount + 6;
    ...
    _cubeDsv = CD3DX12_CPU_DESCRIPTOR_HANDLE(_dsvHeap->GetCPUDescriptorHandleForHeapStart(), 1, _dsvDescriptorSize);
}

void DynamicCubemap::OnKeyboardInput(const GameTimer& timer)
{
...
void DynamicCubemap::AnimateMaterials(const GameTimer& timer)
{
}

void DynamicCubemap::UpdateObjectCBs(const GameTimer& timer)
{
...
void DynamicCubemap::UpdateMaterialBuffer(const GameTimer& timer)
{
...
void DynamicCubemap::UpdateMainPassCB(const GameTimer& timer)
{
    ...
    UpdateCubemapFaceCBs();
}

void DynamicCubemap::UpdateCubemapFaceCBs()
{
    for (int i = 0; i < 6; i++)
    {
        ...
        XMStoreFloat4x4(&cubeFacePassCB.VP, XMMatrixTranspose(viewProj));
        XMStoreFloat4x4(&cubeFacePassCB.InvVP, XMMatrixTranspose(invViewProj));
        ...
        _currFrameResource->PassCB.get()->CopyData(1 + i, cubeFacePassCB);
    }
}
...
void DynamicCubemap::BuildDescriptorHeaps()
{
    ...
    auto rtvCpuStart = _rtvHeap->GetCPUDescriptorHandleForHeapStart();

    int rtvOffset = _swapChainBufferCount;

    CD3DX12_CPU_DESCRIPTOR_HANDLE cubeRtvHandles[6];
    for (int i = 0; i < 6; i++)
        cubeRtvHandles[i] = CD3DX12_CPU_DESCRIPTOR_HANDLE(rtvCpuStart, rtvOffset + i, _rtvDescriptorSize);
...
void DynamicCubemap::BuildCubeDepthStencil()
{
    ...
    ThrowIfFailed(_device->CreateCommittedResource
        (
            ...
            &optClear,
            IID_PPV_ARGS(_cubeDepthStencil.GetAddressOf())
        ));    
    D3D12_DEPTH_STENCIL_VIEW_DESC desc = {};
    desc.Format = _dsvFormat;
    desc.Flags = D3D12_DSV_FLAG_NONE;
    desc.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2D;
    desc.Texture2D.MipSlice = 0;
    _device->CreateDepthStencilView(_cubeDepthStencil.Get(), &desc, _cubeDsv);
    _commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(_cubeDepthStencil.Get(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_DEPTH_WRITE));
}

void DynamicCubemap::BuildShaderAndInputLayout()
{
...
void DynamicCubemap::BuildFrameResources()
{
    for (int i = 0; i < FrameResource::NumFrameResources; i++)
        _frameResources.push_back(std::make_unique<FrameResource>(_device.Get(), 1 + 6, (UINT)_allRenderItems.size(), (UINT)_materials.size()));
}
...
void DynamicCubemap::BuildRenderItems()
{
    ...
    XMStoreFloat4x4(&globeRitem->Model, XMMatrixScaling(2.0f, 2.0f, 2.0f)*XMMatrixTranslation(0.0f, 2.0f, 0.0f));
    XMStoreFloat4x4(&globeRitem->TexTransform, XMMatrixScaling(1.0f, 1.0f, 1.0f));
    globeRitem->ObjCBIndex = 3;
    globeRitem->Mat = _materials["mirror0"].get();
    globeRitem->Geo = _geometries["shapeGeo"].get();
    ...
    _renderItemLayer[(int)RenderLayer::OpaqueDynamicReflectors].push_back(globeRitem.get());
    _allRenderItems.push_back(move(globeRitem));
...
void DynamicCubemap::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& renderItems)
{
...
{
    _commandList->RSSetViewports(1, &_dynamicCubemap->Viewport());
    _commandList->RSSetScissorRects(1, &_dynamicCubemap->ScissorRect());

    _commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(_dynamicCubemap->Resource(), D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_RENDER_TARGET));

    UINT passCbByteSize = D3DUtil::CalcConstantBufferByteSize(sizeof(PassConstants));

    for (int i = 0; i < 6; i++)
    {
        _commandList->ClearRenderTargetView(_dynamicCubemap->Rtv(i), Colors::LightSteelBlue, 0, nullptr);
        _commandList->ClearDepthStencilView(_cubeDsv, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, nullptr);

        _commandList->OMSetRenderTargets(1, &_dynamicCubemap->Rtv(i), true, &_cubeDsv);
    ...
    _commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(_dynamicCubemap->Resource(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_GENERIC_READ));
